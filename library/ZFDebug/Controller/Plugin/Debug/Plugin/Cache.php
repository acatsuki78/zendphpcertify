...
//            print_r( accelerator_get_status());
        ...
        # Support for APC
//        else  if (function_exists('apc_sma_info') && ini_get('apc.enabled')) {
//            $mem = apc_sma_info();
//            $mem_size = $mem['num_seg'] * $mem['seg_size'];
//            $mem_avail = $mem['avail_mem'];
//            $mem_used = $mem_size - $mem_avail;
//            $cache = apc_cache_info();
//            $panel .= '<h4>APC ' . phpversion('apc') . ' Activé</h4>';
//            $panel .= round($mem_avail / 1024 / 1024, 1) . 'M available, ' . round($mem_used / 1024 / 1024, 1) . 'M used<br />' . $cache['num_entries'] . ' Files cached (' . round($cache['mem_size'] / 1024 / 1024, 1) . 'M)<br />' . $cache['num_hits'] . ' Hits (' . round(
//            $cache['num_hits'] * 100 / ($cache['num_hits'] + $cache['num_misses']), 1) . '%)<br />' . $cache['expunges'] . ' Expunges (cache full count)';
//        }
