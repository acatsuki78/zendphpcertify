...
class ZFDebug_Controller_Plugin_Debug_Plugin_Html implements ZFDebug_Controller_Plugin_Debug_Plugin_Interface
{
    ...
    public function getPanel()
    {
        ...
        $panel .= '<span id="ZFDebug_Html_Tagcount"></span> balises<br />' . 'Poids du HTML: ' . round(strlen($body) / 1024, 2) . 'K<br />' . '<span id="ZFDebug_Html_Stylecount"></span> Feuilles de style<br />' . '<span id="ZFDebug_Html_Scriptcount"></span> Fichiers Javascript<br />' .
         '<span id="ZFDebug_Html_Imgcount"></span> Images<br />' . '<form method="POST" action="http://validator.w3.org/check" target="_blank"><input type="hidden" name="fragment" value="' . htmlentities($body) . '"><input type="submit" value="Valider W3C"></form>';
        return $panel;
    }
}